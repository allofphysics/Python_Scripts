...
def html_gen(csv_file,html_file):
    ...
    f=open(html_file,'w')
    f.write('<link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/v/dt/jq-2.2.4/dt-1.10.13/r-2.1.0/datatables.min.css"/>')
    f.write('\n')
    f.write('<script type="text/javascript" src="https://cdn.datatables.net/v/dt/jq-2.2.4/dt-1.10.13/r-2.1.0/datatables.min.js"></script>')
    f.write('\n')
    ...
    tf=tf.fillna('')
    f.write(tf.to_html(escape=False).encode('utf-8').replace('<table border="1" class="dataframe">',strng))
    f.write('\n')
